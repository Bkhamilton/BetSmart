import { getAllValidParticipantBets, getAllValidParticipantBetsWithResults } from '@/db/betslips/ParticipantBets';
import { getAllValidLegs, getAllValidLegsWithResults } from '@/db/betslips/Legs';
...
export const fillBetSlips = async (db, betSlips) => {
    try {
        ...
        const legs = await getAllValidLegsWithResults(db, participantBetIds);
...
// Fill bet slips with bets, legs, and results
export const fillBetSlipsWithResults = async (db, betSlips) => {
    try {
        const betSlipIds = betSlips.map(betSlip => betSlip.id);
        const participantBets = await getAllValidParticipantBetsWithResults(db, betSlipIds);
        const participantBetIds = participantBets.map(participantBet => participantBet.id);
