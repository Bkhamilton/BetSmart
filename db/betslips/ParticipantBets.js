...
export const getAllValidParticipantBetsWithResults = async (db, betSlipIds) => {
    try {
        // Construct placeholders for the array elements
        const placeholders = betSlipIds.map(() => '?').join(',');
        ...
        const query = `
            SELECT 
                ...
                Seasons.leagueId,
                Leagues.leagueName as league,
                ParticipantBetsResults.result as result
            FROM ParticipantBets
            JOIN 
                Games ON ParticipantBets.gameId = Games.gameId
            JOIN 
            ...
            JOIN
                Leagues ON Seasons.leagueId = Leagues.id
            JOIN
                ParticipantBetsResults ON ParticipantBets.id = ParticipantBetsResults.participantBetId
            WHERE betSlipId IN (${placeholders})
        `;
        const allRows = await db.getAllAsync(query, betSlipIds);
        return allRows;
    } catch (error) {
        console.error('Error getting all valid participant bets with results:', error);
        throw error;
    }
};
