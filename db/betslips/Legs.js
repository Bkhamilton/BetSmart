...
// Function to get all Legs for a given array of participantBetIds with results
export const getAllValidLegsWithResults = async (db, participantBetIds) => {
    try {
        // Construct placeholders for the array elements
        const placeholders = participantBetIds.map(() => '?').join(',');
        ...
        const query = `
            ...
            JOIN 
                BetTypes ON Legs.betTypeId = BetTypes.id
            JOIN
                LegsResults ON Legs.id = LegsResults.legId
            WHERE 
                Legs.participantBetId IN (${placeholders})
        `;
